 ...
 * 1. 必须在初始化 SDK 后调用
 ...
 @param codecId 解码ID
 @param codecBackend 解码器后端实现
 @return 0 - 不支持，1 - 支持，2 - 未确定
 @discussion 可以在拉流前调用本接口判断本机是否支持指定解码器
 */
 ...
 * <p><b>注意：</b>
 * 1.视频超分仅对拉流视频画面有效，必须在{@link #initSDK}后调用；<br>
 * 2.该功能需要特殊编包，请联系ZEGO技术支持；<br>
 * 3.该功能会额外耗费系统资源，为了保证用户体验，ZEGO限制只能对一条流开启超分辨率，且该条流的原始分辨率不建议超过 640 × 360；<br>
 * 4.该功能对用户设备有一定要求；<br>
 * 5.通过 [ZegoLivePlayerDelegate -onPlayVideoSuperResolutionUpdate:state:errorCode:]  回调超分状态。
 * </p>
 @param streamID 当前需要开启或关闭超分的流的 ID。
 @param enable 是否开启超分，默认不开启。
 */ 
 ...
 * 
 @param streamID 超分的流 ID
 @param state 超分状态, 0 关闭，1 开启
 @param errorCode 错误码
 */
