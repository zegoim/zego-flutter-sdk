...
/**
 ...
 @param mode 视频朝向模式，详见 ZegoOrientationMode，默认为 ZEGOAPI_ORIENTATION_MODE_CUSTOM。
 @return true 成功，false 失败
 @attention 必须在预览或者推流之前调用
 @attention 对所有通道生效。自适应模式在预览、拉流、混流场景下生效，不支持视频外部采集、媒体播放器、云录制、本地录制，不支持通过 CDN 推拉流场景。
 @see -setAppOrientation:
 */
