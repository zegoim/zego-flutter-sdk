...
typedef enum :  NSUInteger
{
    ...
    ZEGOAPI_MEDIA_RECORD_CHN_THIRD = 2,
    /** 第四路录制通道，即推流时 ZegoAPIPublishChannelIndex 选择了第四路推流通道，录制时采用第四路录制通道 */
    ZEGOAPI_MEDIA_RECORD_CHN_FOURTH = 3
...
/** 媒体录制配置 */
@interface ZegoAPIMediaRecordConfig : NSObject
...
/**
 录制文件配置，单个配置由 ';' 隔开，例如：@"episode_id=XXX;"。目前支持的配置:
 "episode_id": 仅支持 Mp4 文件格式
 */
 ...
 * 注意：必须在初始化 SDK之后调用。
 ...
 @param storagePath 录制文件存储路径，传入的路径需要包含文件名
 ...
 * 2.若需要录制 ZEGOAPI_MEDIA_RECORD_AUDIO 类型媒体，在开始录制之前必须开启麦克风或者带麦克风的摄像头，不开启则不能正常返回录制的音频文件；同理，录制 ZEGOAPI_MEDIA_RECORD_VIDEO 类型媒体，必须开启摄像头，录制 ZEGOAPI_MEDIA_RECORD_BOTH 类型媒体，麦克风和摄像头都需要开启。
 ...
 @param storagePath 录制文件存储路径，传入的路径需要包含文件名，存储的路径必须是应用有权限读写的，如果传入目录路径， - onMediaRecord:channelIndex:storagePath: 回调会返回写文件失败。
 ...
 * 2.若需要录制 ZEGOAPI_MEDIA_RECORD_AUDIO 类型媒体，在开始录制之前必须开启麦克风或者带麦克风的摄像头，不开启则不能正常返回录制的音频文件；同理，录制 ZEGOAPI_MEDIA_RECORD_VIDEO 类型媒体，必须开启摄像头，录制 ZEGOAPI_MEDIA_RECORD_BOTH 类型媒体，麦克风和摄像头都需要开启。
 ...
 @param interval 录制文件时状态回调的间隔，单位毫秒，有效范围：[1000,10000]，此参数决定了 - onRecordStatusUpdateFromChannel:index storagePath:duration:fileSize: 的回调频率。
 @param recordFormat 录制文件的文件格式，详见 enum ZegoAPIMediaRecordFormat。
 @return true 调用成功，false 调用失败
 @see - stopRecord:
 */
 ...
 * 2.若需要录制 ZEGOAPI_MEDIA_RECORD_AUDIO 类型媒体，在开始录制之前必须开启麦克风或者带麦克风的摄像头，不开启则不能正常返回录制的音频文件；同理，录制 ZEGOAPI_MEDIA_RECORD_VIDEO 类型媒体，必须开启摄像头，录制 ZEGOAPI_MEDIA_RECORD_BOTH 类型媒体，麦克风和摄像头都需要开启。
 ...
 @param config 媒体录制配置，详见 ZegoAPIMediaRecordConfig
 @return true 调用成功，false 调用失败
 */
