...
class PublishStreamPlatformViewPage extends StatefulWidget {
  ...
  PublishStreamPlatformViewPage(this.screenWidthPx, this.screenHeightPx);

  @override
  _PublishStreamPlatformViewPageState createState() =>
      new _PublishStreamPlatformViewPageState();
}

class _PublishStreamPlatformViewPageState
    extends State<PublishStreamPlatformViewPage> {
  String _title = '';
  int _viewID = -1;
  ...
  void initState() {
    ...
    _publishView =
        ZegoLiveRoomPublisherPlugin.createPreviewPlatformView((int viewID) {
      _viewID = viewID;
      print('view ID : $_viewID');
      ZegoLiveRoomPublisherPlugin.setPreviewView(_viewID);
      ZegoLiveRoomPublisherPlugin.setPlatformViewPreviewViewMode(
          ZegoViewMode.ZegoRendererScaleAspectFill);
      ZegoLiveRoomPublisherPlugin.startPreview();
    });

    print('publish view key: ${_publishView.key.toString()}');
  }
  ...
  void dispose() {
    ...
    ZegoLiveRoomPublisherPlugin.removePreviewPlatformView(_viewID);
